...
from ..models import (Account, BankAccount, BankTransaction, Company, Currency,
                      Invoice, InvoiceLine, Item)
from ..services import apply_bank_tx_to_inv, open_invoice, pay_invoice
...
class InvoiceLifecycleTests(TestCase):
    ...
    def make_invoice(self, lines=None, total=Decimal("0.00"),
    ...
    def test_open_invoice_requires_at_least_one_line(self):
        ...
        # calling open_invoice() on an invoice
        # with no lines should raise ValidationError
    ...
    def test_successful_open_and_pay_flow_and_paid_immutability(self):
        ...
        apply_bank_tx_to_inv(
            self.bt.id,
            [{"invoice_id": invoice.id, "amount": invoice.outstanding_amount}],
        )
