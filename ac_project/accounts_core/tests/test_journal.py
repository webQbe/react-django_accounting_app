...
from accounts_core.models import (Account, Company, Currency, JournalEntry,
                                  JournalLine, User)
...
class JournalEntrySuccessTests(TestCase):
    ...
    def setUp(self):
        ...
        self.je = JournalEntry.objects.create(
            company=self.company, date=datetime.date.today(), status="draft"
        )
    ...
    """ Test for Idempotency: 1. You create a balanced journal entry.
    ...
    def test_post_is_idempotent_when_called_twice_with_same_data(self):
        ...
        first_lines = list(
            self.je.lines.values_list(
                "id", "debit_original", "credit_original")
        )
        ...
        # only 2 lines, no duplicates
        self.assertEqual(self.je.lines.count(), 2)
...
class JournalEntryFailureTests(TestCase):
    ...
    def setUp(self):
        ...
        self.cash_a = Account.objects.create(
        ...
        self.je = JournalEntry.objects.create(
            company=self.company1, date=datetime.date.today(), status="draft"
        )
    ...
    def test_post_atomicity_on_failure(self):
        ...
        """ Checks to confirm the entry was not transformed
        into a posted/immutable state. """
        # 1) Ensure JE was not marked as posted
        self.assertEqual(
            self.je.status,
            "draft", "JournalEntry.status should remain 'draft'"
        )
        ...
        self.assertEqual(
            after,
            before, "JournalLine count should not change after failed post()"
        )
    ...
    def test_post_enforces_tenant_scope(self):
        ...
        self.company2 = Company.objects.create(
            name="Test Co 2", slug="test_co2", default_currency=self.usd
        )
        ...
        # Creating a JournalLine whose company
        # differs from the JournalEntry's company
        ...
        Refresh JE and assert posting did not happen
        and input lines unchanged """
        self.je.refresh_from_db()
    ...
    def test_post_raises_if_already_posted_and_data_changed(self):
        ...
        # change persisted DB rows WITHOUT calling model.save()
        ...
        # Clear any prefetched cache on the journal
        #  so post() reads fresh rows from DB
        if hasattr(self.je, "_prefetched_objects_cache"):
            self.je._prefetched_objects_cache.pop("lines", None)

        # Now posting again should detect
        # the persisted payload differs and raise
...
class JournalEntryFreezeTests(TestCase):
    ...
    def make_balanced_entry(self):
        ...
        JournalLine.objects.create(
            company=self.company, journal=je, account=self.asset,
            debit_original=100
        )
        JournalLine.objects.create(
            company=self.company, journal=je,
            account=self.revenue, credit_original=100
        )
        return je

    def test_post_freezes_lines_on_update__raises_validationerror(self):
        """Typical design:
        line.save() should raise ValidationError for posted entries."""
    ...
    def test_post_prevents_line_deletion(self):
        """Deleting a posted line should be prevented
        (ProtectedError or ValidationError)."""
        je = self.make_balanced_entry()
        je.post()
