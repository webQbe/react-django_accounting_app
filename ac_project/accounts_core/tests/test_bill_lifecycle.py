...
from ..models import (Account, BankAccount, BankTransaction, Bill, BillLine,
                      Company, Currency, Item)
from ..services import apply_bank_tx_to_bill, pay_bill, post_bill
...
class BillLifecycleTests(TestCase):
    def setUp(self):
        ...
        self.bt = BankTransaction.objects.create(
            ...
            payment_date=datetime.date(2025, 9, 18),
            amount=Decimal("100.00"),
            currency_code="USD",
        )

    def make_bill(self, lines=None, total=Decimal("0.00"),
                  outstanding_amount=None):
        """
        Helper to create an bill.
        `lines` is a list of (description, amount) tuples.
        """
    ...
    def test_post_bill_requires_at_least_one_line(self):
        bill = self.make_bill(
            lines=None, total=Decimal("100.00"),
            outstanding_amount=Decimal("100.00")
        )

        # calling post_bill() on an bill with
        # no lines should raise ValidationError
    ...
    def test_successful_post_and_pay_flow_and_paid_immutability(self):
        ...
        apply_bank_tx_to_bill(
            self.bt.id,
            [{"bill_id": bill.id, "amount": bill.outstanding_amount}]
        )
        # calls full_clean() & raises ValidationError if it would be invalid
        bill.save()
    ...
    def test_invalid_transition_directly_raises(self):
        ...
        # Trying to go straight from draft ->
        # paid should be forbidden by transition_to
        with self.assertRaises(ValidationError):
