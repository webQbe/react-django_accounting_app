...
from accounts_core.models import Company, Account, Invoice, JournalEntry, JournalLine, BankTransaction, Currency, Customer, BankAccount
...
class Command(BaseCommand):
    help = "Create a demo tenant (company), user, and sample financial data for testing."
    ...
    def handle(self, *args, **options):
        ...
        def unique_slug_for_company(name, max_tries=100):
            ...
            base = slugify(name) or "company" # fall back to "company" if empty string is returned
            slug = base
            i = 1 # add numbers if needed
            # If plain slug is taken, append -1, -2, etc.
            while Company.objects.filter(slug=slug).exists():
                slug = f"{base}-{i}"  # Example: "test-ltd" → "test-ltd-1" → "test-ltd-2"
                i += 1                # Move to next number if slug is still not unique
                if i > max_tries:     # if we try 100 times and still can’t find a free slug
                    raise RuntimeError("Couldn't generate unique slug") # bail out with an error
            return slug
        ...
        company, _ = Company.objects.get_or_create( # ignore `created` boolean flag by naming it `_`
        ...
        user.save()
        self.stdout.write(self.style.SUCCESS(f"Created user: {user.username} (pw={password})"))

        # 3. Create sample accounts
        cash, _ = Account.objects.get_or_create(company=company, code="1110", defaults={"name": "Cash", "ac_type":"Asset", "normal_balance":"debit"})
        revenue, _ = Account.objects.get_or_create(company=company, code="4000", defaults={"name": "Revenue", "ac_type":"Income", "normal_balance":"credit"})
        ...
        JournalLine.objects.create(journal=journal, company=company, account=cash, currency=usd, debit_original=Decimal("1000.00"), credit_original=Decimal("0.00"))
        JournalLine.objects.create(journal=journal, company=company, account=revenue, currency=usd, debit_original=Decimal("0.00"), credit_original=Decimal("1000.00"))
        journal.post()  
