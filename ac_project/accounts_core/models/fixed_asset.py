...
class FixedAsset(
    models.Model
):  # tracks long-term assets and handle depreciation over time
    ...
    """ Example: if a $12,000 asset is depreciated $4,000 per year,
    after 2 years this field = $8,000. """
    ...
    def clean(self):
        ...
        # Prevent assets from being marked as depreciable
        # when their lifespan hasn’t been set
        # Without a positive number of years, depreciation makes no sense
        if self.depreciation_method and (
            not self.useful_life_years or self.useful_life_years <= 0
        ):
            raise ValidationError(
                "Useful life must be > 0 if depreciation method is set"
            )
