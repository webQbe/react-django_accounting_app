...
from accounts_core.models import User, Invoice, InvoiceLine
...
class InvoiceLineForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if getattr(self.instance, "invoice_id", None) and not getattr(self.instance, "company_id", None):
            self.instance.company_id = Invoice.objects.only("company_id").get(pk=self.instance.invoice_id).company_id
        return super().clean()
