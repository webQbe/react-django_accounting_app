...
from accounts_core.models import (BankAccount, BankTransaction,
                                  BankTransactionBill, BankTransactionInvoice,
                                  Currency)
...
class BankTransactionInvoiceAdmin(TenantAdminMixin, admin.ModelAdmin):
    list_display = (
        "id", "company", "bank_transaction", "invoice", "applied_amount")
...
class BankTransactionBillAdmin(TenantAdminMixin, admin.ModelAdmin):
    list_display = (
        "id", "company", "bank_transaction", "bill", "applied_amount")
    list_filter = ("company", "bank_transaction")
...
class CurrencyAdmin(TenantAdminMixin, admin.ModelAdmin):
    ...
    ordering = ("code",)
    # set pagination so that only 50 currencies show per page
    list_per_page = 50
